...
static void startTransitionFadeOut(engineDevice* device)
{
...
static void updateFadeIn(engineDevice* device, f32 speed, f32 current_time)
{
	float difference = (current_time - transition_time_start) / 1000;
	IVideoDriver* driver = device->getVideoDriver();
	driver->draw2DRectangle(engine::video::SColor(transition_alpha, 0, 0, 0),
		rect<int>(0, 0, driver->getScreenSize().Width, driver->getScreenSize().Height));
...
static void updateFadeOut(engineDevice* device, f32 speed, f32 current_time)
{
	float difference = (current_time - transition_time_start) / 1000;
	IVideoDriver* driver = device->getVideoDriver();
	driver->draw2DRectangle(engine::video::SColor(transition_alpha, 0, 0, 0),
...
static vector3df getSceneNodeFromScreenCoordinatesBB(ISceneManager* smgr, IVideoDriver* driver, ITriangleSelector* sel, position2d<int> pos, int idBitMask)
{
   ...
   vector3df end = farLeftUp + (lefttoright * dx) + (uptodown * dy);
...
static LIGHTNING_TYPE lightning_types[]={
		{"media/lightning/blue_bolt.png", "media/shields/shield_blue.png", SColorf(0, 0.2f, 0.2f), 2, 0.1f, 0.25f},
		{"media/lightning/yellow_bolt.png", "media/shields/shield_yellow.png", SColorf(0.2f, 0.2f, 0), 2, 0.2f, 0.2f},
		{"media/lightning/green_bolt.png", "media/shields/shield_green.png", SColorf(0, 0.2f, 0), 3, 0.2f, 0.14f},
		{"media/lightning/purple_bolt.png", "media/shields/shield_purple.png", SColorf(0.2f, 0, 0.2f), 6, 0.2f, 0.08f},
		{"media/lightning/red_bolt.png", "media/shields/shield_red.png", SColorf(0.2f, 0, 0), 9, 0.4f, 0.03f} };